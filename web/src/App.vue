<template>
  <transition name="loadingOut">
    <div v-if="loading" id="loading" style="position:relative;width:100vw;height:100vh;background-color: black;" @click="loadingTohome">  
      <div style="position:absolute;top:40vh;left:28vw;font-size: 7vh;color: white;filter: contrast(15);" id="placeFirst"></div>
      <div style="position:absolute;top:50vh;left:35vw;font-size: 7vh;color: white;filter: contrast(15);" id="featureFirst"></div>
    </div>
  </transition>
  <transition name="homePage">
    <div id="homePage" v-if="homePage" style=" position: absolute;">
      <main>
          <n-space style="position:absolute;left:40vw;top:45vh;z-index: 100;">
  ...
  <div class="g-container" v-if="rule" @click="switchStatement">
    ...
    <n-button type="tertiary" @click="toMainPage">skip</n-button>
   </div>
  </transition>
  <n-space id="mainPage" vertical  v-if="mainPage" ref="mainPage">
    <n-layout-header style="position:relative;z-index: 1000000;border-radius: 2vh;" ref="header">
        ...
        <n-button quaternary style="right:28vw" @click="toHomePage"><img src="./assets/images/house-fill.svg">Home</n-button>
        <n-input v-on:keyup="searchRelative" v-model:value="value" type="text" placeholder="输入可能正确的景点......" />
        <n-button type="tertiary" @click="submit">
            提交
        </n-button>
        <n-button quaternary style="left:20vw"><img src="./assets/images/boxes.svg" >rank</n-button>
        <n-button quaternary style="left:26vw" @click="comment=true"><img src="./assets/images/chat-square-text.svg">comment</n-button>
      </n-space>
    ...
    <n-layout content-style="padding: 0px;border-radius: 2vh;height:80vh" has-sider sider-placement="left">
      ...
      <div id="result" v-if="card" style="padding:10vh 10vw">
          <n-card hoverable closable @close="this.card = false" style="width:80vw;height: 60vh;border-radius: 2vh;">
              <template #header-extra>
                  <n-button-group>
                      <n-button ghost @click="displayRelationM(this.id)"> result </n-button>
    ...
    <n-layout-footer style="border-radius: 2vh;" ref="footer">
      ...
      <a href="https://github.com/landleHUST/landle" target="_blank" style="position:absolute;left:95vw;"><img style="width:5vh;" src="./assets/images/github.svg"></a>
      </n-space>
    </n-layout-footer>
  </n-space>
</template>
<script>
import {ref} from "vue";
import {displayMap} from "./function"
import {displayRelation} from "./function"
...
export default{
  ...
  methods:{
    displayRelationM(id){
      ...
      this.id=id;
      console.log(this.id);
      displayRelation(this.id,correctId,placeList,featureList,this.correctFeatureList,myChart);
    },
    displayIntroduciton() {
